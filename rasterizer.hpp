...
namespace rst
{
    ...
    struct pos_buf_id
    {
		pos_buf_id(int v) { pos_id = v; }
        int pos_id = 0;
    };

    struct ind_buf_id
    {
		ind_buf_id(int v) { ind_id = v; }
        int ind_id = 0;
    };

    struct col_buf_id
    {
		col_buf_id(int v) { col_id = v; };
        int col_id = 0;
    };
    ...
    public:
		...
		void isvisible(const Triangle & t, const Triangle & dt);
		...
		//int isvisible(const Triangle & t);
